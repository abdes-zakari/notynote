...
class Category extends Component {
  ...
  render() {
   ...
   let listCat= this.state.categories.map((cat, index) =>
    <div style={{backgroundColor:'#434956',color:'#F0F0F0',padding:"10px",margin:"10px",textAlign:"center"}} key={index} > 
        {cat.category}
        <span className="delete-category" onClick={() => this.onRemoveCategory(cat.id)}> 
    ...
    <div className={classes.root}>
      <Grid container spacing={24}>
        ...
        <Grid item xs={8}  className={classes.inside}>
          ...
          <div>
            <InputText value={this.state.new_category} onChange={this.handleNewCategory} />
            <ButtonSave submitProps={this.handleSubmit}/>
            <Masonry
                breakpointCols={2}
